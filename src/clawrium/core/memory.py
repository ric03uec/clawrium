...
# boot and self-deletes after use, so a clawctl-side write would corrupt the
# bootstrap protocol (ATX round 1 W3 for #358).
#
# Intent: these allowlists are STRUCTURAL, not phase-gated. clawctl has no
...
# BOOTSTRAP.md as permanently unwritable from clawctl avoids ever having to
...
# never a SOUL.md delete via clawctl — the file lives outside ``memories/``
