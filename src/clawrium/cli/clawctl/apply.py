...
def apply(
    ...
    yes: bool = typer.Option(
        ...
        "--yes",
        "-y",
        help="Skip confirmation on destructive changes.",
    ),
) -> None:
    ...
    if missing_secrets:
        ...
        for s in missing_secrets:
            # s is like "providers/ethos-openrouter/apiKey"
            parts = s.split("/")
    ...
    errors = execute_apply(doc, cs, actual, emit=emit, dry_run=False)
...
def _print_changeset(cs, dry_run: bool = False) -> None:
    ...
    for op in cs.updates:
        action = f"would update ({op.details})" if dry_run else f"update ({op.details})"
        typer.echo(f"{prefix} {op.kind}/{op.name}  {action}")
