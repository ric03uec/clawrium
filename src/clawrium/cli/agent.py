...
def _run_channels_stage(
    ...
    """Deprecated CHANNELS onboarding stage — always exits with code 2.

    Channel state is canonical in `channels.json` (with secrets in
    `secrets.json`); the operator is redirected at the modern
    `clawctl channel` surface. The function remains wired into the
    legacy driver's STAGES dispatchers so `--stage channels` still
    resolves; full removal is tracked by #707.
    """
