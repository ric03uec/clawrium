...
def _safe(text: str) -> str:
    """Sanitize + Rich-escape a remote/exception string in one call.

    Delegates to the canonical ``cli.output._sanitize.sanitize`` (strips
    C0/C1 control bytes and UAX#9 bidi + zero-width codepoints), then
    Rich-escapes so residual ``[`` / ``]`` in the payload cannot open a
    markup span. Issue #453 — closes the bidi + markup-injection gap on
    lifecycle event and exception rendering.
    """
    return rich_escape(sanitize(str(text)))
    ...
    if stage == "gateway_token_rotated":
        ...
        if agent_label:
            console.print(
                ...
                f"{_safe(agent_label)}. Active chat sessions on "
...
def _run_providers_stage(
    ...
    try:
        ...
        console.print(
            f"[red]Error:[/red] Providers file corrupted: {_safe(str(e))}"
        )
        return False
    ...
    try:
        ...
        console.print(f"[red]✗[/red] {_safe(str(e))}")
...
def _run_identity_stage(
    ...
    if identity_files:
        ...
        for src_file in identity_files:
            ...
            try:
                ...
                console.print(
                    f"[red]Error:[/red] Failed to copy {src_file.name}: {_safe(str(e))}"
                )
                return False
        ...
        if not soul_path.exists():
            ...
            try:
                ...
                console.print(
                    f"[red]Error:[/red] Failed to write SOUL.md: {_safe(str(e))}"
                )
                return False
    else:
    ...
    try:
        ...
        if success:
            ...
            console.print(
                f"  [yellow]Warning:[/yellow] Workspace sync failed: {_safe(str(error))}"
            )
            console.print(
        ...
        console.print("[red]✗[/red]")
        console.print(
            f"  [yellow]Warning:[/yellow] Workspace sync failed: {_safe(str(e))}"
        )
        console.print(
            "  [dim]Identity files saved locally. Sync later with 'clawctl agent sync'[/dim]"
        )
    ...
    if current_state == "identity":
        try:
            ...
            console.print(
                f"[red]Error:[/red] Failed to save identity stage: {_safe(str(e))}"
            )
            return False
...
def _run_channels_stage(
    ...
    if channels_status == "complete":
        ...
        try:
            ...
            console.print(
                f"[red]✗[/red] Failed to update channel metadata: {_safe(str(e))}"
            )
            return False
    ...
    try:
        ...
        console.print(
            f"[red]Error:[/red] Failed to save channels stage: {_safe(str(e))}"
        )
        return False
...
def _run_validate_stage(
    ...
    if install_result.passed:
        ...
        for error in install_result.errors:
            console.print(f"    [red]Error:[/red] {_safe(error)}")
    ...
    if provider_result.passed:
        ...
        console.print(
            f"  [green]✓[/green] Provider: {_safe(provider_id)} ({_safe(provider_type)})"
        )
    ...
    if claw_type == "openclaw":
        ...
        if skip_health:
            ...
            if conn_result.passed:
                ...
                if endpoint:
                    console.print(
                        f"  [green]✓[/green] Gateway reachable: {_safe(str(endpoint))}"
                    )
                else:
    ...
    try:
        ...
        console.print(
            f"[red]Error:[/red] Failed to save validate stage: {_safe(str(e))}"
        )
        return False
...
def _run_edit_config(
    ...
    try:
        ...
        try:
            ...
            console.print(f"\n[red]Error:[/red] Invalid JSON: {_safe(str(e))}")
        ...
        if not success:
            console.print(f"\n[red]Error:[/red] Sync failed: {_safe(str(error))}")
            console.print("\nYour edited file has been preserved at:")
            console.print(f"  {config_file}")
            preserve_temp_dir = True
            raise typer.Exit(code=1)
        ...
        if typer.confirm("\nRestart agent to apply changes?", default=True):
            ...
            try:
                ...
                console.print(f"[red]Error:[/red] Restart failed: {_safe(str(e))}")
                raise typer.Exit(code=1)
        else:
            console.print(
...
def configure(
    ...
    try:
        ...
        console.print(f"[red]Error:[/red] {_safe(str(e))}")
        raise typer.Exit(code=1)
        ...
        for i, stage_name in enumerate(stage_order):
            ...
            if can_skip_stage(claw_type, stage_name):
                ...
                try:
                    ...
                    console.print(
                        f"[red]Error:[/red] Failed to skip stage: {_safe(str(e))}"
                    )
                    raise typer.Exit(code=1)
...
def open_ui(
    ...
    try:
        ...
        err_console.print(f"[red]Error:[/red] {_safe(str(e))}")
        raise typer.Exit(code=1)
    except HostsFileCorruptedError as e:
        err_console.print(f"[red]Error:[/red] {_safe(str(e))}")
        raise typer.Exit(code=1)
...
def integrations_add(
    ...
    try:
        ...
        console.print(
            ...
            f"'clawctl agent integration remove {_safe(claw_name)} {_safe(e.existing_name)}'.[/dim]"
        )
        raise typer.Exit(code=1)
