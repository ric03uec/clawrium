...
class TestInitialize:
    ...
    def test_skips_rewrite_when_identical(self, _hook, tmp_path: Path) -> None:
        ...
        version_file = tmp_path / "src" / "clawrium" / "_version.py"
        # Write in the exact format that initialize() produces (!r -> single quotes)
        version_file.write_text(
        ...
        _hook.initialize("1.0.0", {})
        # On some CI tmpfs filesystems (e.g. ubuntu-latest) the mtime can drift
        # by a few tens of nanoseconds even when no write occurs.  Check within
        # a 1-second tolerance to stay flake-free while still catching a rewrite.
        final_mtime = version_file.stat().st_mtime_ns
