...
def test_parse_file_all_five_kinds(tmp_path: Path) -> None:
    """parse_file correctly parses a multi-doc YAML with all 5 resource kinds."""
...
def test_parse_file_unknown_kind_raises(tmp_path: Path) -> None:
    """parse_file raises ValueError for an unknown resource kind."""
...
def test_parse_directory(tmp_path: Path) -> None:
    """parse_directory assembles resources from all YAML files in order."""
...
# ── test 2: validate_refs catches missing host ────────────────────────────────

def test_validate_refs_missing_host(tmp_path: Path) -> None:
    """validate_refs returns an error when an agent refs a host not in the manifest."""
...
def test_validate_refs_missing_provider(tmp_path: Path) -> None:
    """validate_refs catches a missing provider ref."""
    f = _write_yaml(tmp_path, "fleet.yaml", """\
        apiVersion: clawrium.io/v1
        kind: Host
        metadata:
...
def test_validate_refs_ok(tmp_path: Path) -> None:
    """validate_refs returns empty list for a self-consistent manifest."""
    f = _write_yaml(tmp_path, "fleet.yaml", FLEET_YAML)
    doc = parse_file(f)
    errors = validate_refs(doc)
    assert errors == []
...
# ── test 3: compute returns correct creates for a fresh fleet ─────────────────

def test_compute_creates_for_fresh_fleet(tmp_path: Path) -> None:
    """compute produces create ops for every resource when actual state is empty."""
    f = _write_yaml(tmp_path, "fleet.yaml", FLEET_YAML)
    doc = parse_file(f)
    ...
    # Attachments should also appear for the new agent
    ...
    # autoStart → agent should appear in starts
    assert "my-agent" in cs.starts


def test_compute_no_deletes_on_apply(tmp_path: Path) -> None:
    """compute(for_delete=False) never produces deletes."""
    f = _write_yaml(tmp_path, "fleet.yaml", FLEET_YAML)
    doc = parse_file(f)
    actual = _make_empty_actual()
    cs = compute(doc, actual)
...
# ── test 4: idempotent — second apply returns all noops ──────────────────────

def test_compute_idempotent_noop(tmp_path: Path) -> None:
    """compute returns only noops when actual state already matches the manifest."""
    f = _write_yaml(tmp_path, "fleet.yaml", FLEET_YAML)
    doc = parse_file(f)

    # Simulate the "already applied" state
    actual = ActualState(
        hosts={
...
# ── test 5: compute attaches / detaches on diff ───────────────────────────────

def test_compute_attach_new_channel(tmp_path: Path) -> None:
    """compute produces an attach op when a new channel is added to an existing agent."""
    f = _write_yaml(tmp_path, "fleet.yaml", FLEET_YAML)
    doc = parse_file(f)

    # Agent exists but without the discord channel
    actual = ActualState(
        hosts={
...
def test_compute_detach_removed_integration(tmp_path: Path) -> None:
    """compute produces a detach op when an integration is removed from the manifest."""
    # Manifest without gh-main integration on the agent
    f = _write_yaml(tmp_path, "fleet.yaml", """\
        apiVersion: clawrium.io/v1
        kind: Host
        metadata:
    ...
    # Actual state has the integration attached
    actual = ActualState(
        hosts={
            "mybox": {"hostname": "192.168.1.100"},
            "192.168.1.100": {"hostname": "192.168.1.100"},
        ...
        agents={
            "my-agent": ActualAgent(
                ...
                providers=[],
                channels=[],
                integrations=["gh-main"],  # should be removed
                skills=[],
            )
        },
    )
...
def test_compute_delete_for_delete_flag(tmp_path: Path) -> None:
    """compute(for_delete=True) produces delete ops for agents that exist."""
    f = _write_yaml(tmp_path, "fleet.yaml", """\
        apiVersion: clawrium.io/v1
        kind: Host
        metadata:
    ...
    # Creates/attaches should be empty in delete mode
    assert cs.creates == []
    assert cs.attaches == []


def test_compute_delete_noop_for_absent_agent(tmp_path: Path) -> None:
    """compute(for_delete=True) produces a noop if the agent doesn't exist."""
    f = _write_yaml(tmp_path, "fleet.yaml", """\
        apiVersion: clawrium.io/v1
        kind: Agent
        metadata:
...
# ── test: collect_secret_refs ─────────────────────────────────────────────────

def test_collect_secret_refs(tmp_path: Path) -> None:
    """collect_secret_refs extracts all secretRef paths from the document."""
    f = _write_yaml(tmp_path, "fleet.yaml", FLEET_YAML)
    doc = parse_file(f)
...
def test_collect_secret_refs_empty_when_no_credentials(tmp_path: Path) -> None:
    """collect_secret_refs returns empty list when no credentials are defined."""
    f = _write_yaml(tmp_path, "fleet.yaml", """\
        apiVersion: clawrium.io/v1
        kind: Host
        metadata:
