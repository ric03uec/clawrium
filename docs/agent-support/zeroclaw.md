...
| **Onboarding wizard** | ✅ | 4 stages: `providers` (required) → `identity` (auto-skipped) → `channels` (required, CLI confirm) → `validate` (local checks: hosts.json record, provider config, API key, connectivity). |
...
| **validate** | Local validation only: confirms the hosts.json record, provider config, API key, and provider connectivity. The configure playbook's own readiness probe is `GET /health/providers` (separate from this stage). The manifest defines a `binary_check` task (`zeroclaw --version`) that is not dispatched yet — remote version verification is planned. |
...
- **For untrusted networks, prefer a direct SSH tunnel + raw WebSocket client.** `clm chat` rebuilds the gateway URL from the host record's primary address in `hosts.json` (see [Off-host access from the control machine](#off-host-access-from-the-control-machine) above), so a simple `ssh -L 42617:127.0.0.1:42617` does not redirect `clm chat`. For ad-hoc verification, point a raw client (`wscat`, `websocat`) at the tunnelled loopback instead.
- **`ws://` carries the bearer token in cleartext on non-loopback connections.** `core/chat_zeroclaw.py` warns once per session when the destination is non-loopback (`_warn_if_token_in_cleartext`). Production deployments should terminate TLS at a reverse proxy (Caddy / nginx) and use a `wss://` URL; clm does not run that proxy itself.
